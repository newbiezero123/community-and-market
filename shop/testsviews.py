    ...
    if request.method == "POST":
        ...
        if quantity > 0:
            ...
            if cart_item.product.category == 'clothing':
                cart_item.size = size
            cart_item.save()
            messages.success(request, f"อัปเดตสินค้า {cart_item.product.name} เรียบร้อยแล้ว")
        else:
            cart_item.delete()
...
def sales_report(request):
    ...
    sales_today = OrderItem.objects.filter(order__created_at__date=today).aggregate(Sum('price'))['price__sum'] or 0
    sales_last_7_dayssum = OrderItem.objects.filter(order__created_at__date__gte=seven_days_ago).aggregate(Sum('price'))['price__sum'] or 0
    sales_this_month = OrderItem.objects.filter(order__created_at__date__gte=start_of_month).aggregate(Sum('price'))['price__sum'] or 0
